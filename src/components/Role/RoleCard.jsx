...
const RoleCard = ({ role, userRole }) => {
    console.log(userRole);

    return (
        <div
            className={`p-6 cursor-pointer rounded-2xl border
                        ${role === userRole ? 'border-primary bg-secondary' : ' bg-gray-100 border-[rgba(223,227,232,1)]  '} space-y-5`}>
            ...
            <div className='gap-2 text-center max-w-[240px] mx-auto'>
                <h2 className={`
                ${role === userRole? 'text-primary ' : 'text-gray-600 '}
                 text-2xl font-semibold `}>
                    {roleData[role].role}
                </h2>
                <p className={`${role === userRole ? 'text-primary ' : 'text-gray-600 '}
